...
public class PizzaShopService {
    ...
    public List<Pizza> getPizzas() {
        ...
        for (DamenosPizza damenosPizza: damenosService.getPizzas()) {
            pizzas.add(new DamenosAdapter(damenosPizza));
        }

        for (PizzaHotPizza pizzaHotPizza: pizzaHotService.getPizzas()) {
            pizzas.add(new PizzahotAdapter(pizzaHotPizza));
        }
